main:
    li sp, 1024                 # simulated stack 
    addi sp, sp, -8             # allocate 8 bytes on the stack
    li t0, '6'                  # store "65535\0"
    sb t0, (sp)
    li t0, '5'
    sb t0, 1(sp)
    sb t0, 2(sp)
    ...
    mv t0, zero
    sb t0, 5(sp)
    mv a0, sp                   # set up string in a0
    li a1, 10                   # set up base in a1
    jal str_to_int_s            # expected: a0 contains 0xffff
    unimp
...
si_len_done:
    ...
    jal calculate               # a0 = str at least sig place, a1 = base, a2 = len
