...
uint32_t cache_lookup_dm(struct cache_st *csp, uint64_t addr) {
    ...
    b_index =  (addr >> 2) & 0x3;// change here
    ...
    if (slot->valid && (slot->tag == tag)) {
        ...
        for (int i = 0; i< 4;i++){
       		slot -> block[i] = *((uint32_t *)(base_addr + (i * 4)));
        }
...
uint32_t cache_lookup_sa(struct cache_st *csp, uint64_t addr) {
    ...
    uint64_t b_index = (addr >> 2)& ((1 << csp->block_bits) - 1); // Need to change for block size > 1
    ...
    uint64_t min_timestamp = UINT64_MAX;  // Track least recently used slot
    ...
    for (int i = 0; i < 4; i += 1) {
        ...
        if (slot->valid) {
			...
			// Track the least recently used slot
    ...
    if (!hit) {
       // Load entire block for 4 words
       uint32_t *block_addr = (uint32_t *)(addr & ~(uint64_t)(3 * sizeof(uint32_t)));
       for (int i = 0; i < 4; i++) {
       		slot->block[i] = block_addr[i];
       }
